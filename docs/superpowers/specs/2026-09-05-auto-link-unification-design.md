...
- **Feedback** (`silent` mode, new default): replace `whisperSummary` with `notifyRetroResult(entities, applied, rows, { writable, failed })`:
  ...
  - `failed > 0` (a page write threw or the page vanished) → `ui.notifications.error` with `retroLink.writeFailed` ("Auto-link could not write {count} page(s) — see the browser console (F12)."), detail logged with `console.error`.
