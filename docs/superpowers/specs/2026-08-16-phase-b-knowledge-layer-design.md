...
A collapsible section appended to the bottom of every MEJ sheet. No libWrapper needed (refined from the original design after reading MEJ's shell): MEJ's shell already fires `Hooks.callAll("renderJournalPageSheet", subsheet, subsheetElement, {enhancedjournal, …})` at the end of `renderSubSheet` (apps/enhanced-journal.js:691), and popped-out sheets fire the standard `renderEnhancedJournalSheet` ApplicationV2 render hook — the companion injects from those two hooks, idempotently. Each row: mentioning entry's type icon, name, mention count; click opens it as a normal MEJ tab. Hub index rows show a mention-count badge.
